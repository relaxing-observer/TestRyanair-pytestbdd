...
def main_page(browser, user, password, main):
    user_main_page = PageConstructor.get_class(main)(browser, Links.get_page_link(main))
    user_main_page.open()
...
def input_depart_date(browser, depart_date, flights):
    flight_page = PageConstructor.get_class(flights)(browser, browser.current_url)
    flight_page.select_depart_date(depart_date)


@when(parsers.cfparse('I choose "{return_date}" at Return Form on "{flights}" tab'))
def input_return_date(browser, return_date, flights):
    flight_page = PageConstructor.get_class(flights)(browser, browser.current_url)
    flight_page.select_return_date(return_date)
    flight_page.go_search()


@then(parsers.cfparse(
    'I should see flight cards from "{departure_airport}" to "{destination_airport}" at "{depart_date}" and "{'
    'return_date}" at "{search_flights}" page'))
